...
namespace PLI.System.API.Controllers.V1
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create(UserCreateViewModel model, CancellationToken cancellationToken)
        {
            ...
            {
                string message = "";
                if (await _userService.IsExists("UserName", model.EmployeeId, cancellationToken))
                {
                    message = $"The user name- '{model.EmployeeId}' already exists";
            ...
            return StatusCode(StatusCodes.Status400BadRequest, new ResponseViewModel<UserViewModel>
            {
                ...
                Error = new ErrorViewModel
                {
                    ...
                    Message = ModelStateHelpers.GetErrors(ModelState)
                }
            });
        }
        ...
        public async Task<IActionResult> Edit(UserUpdateViewModel model, CancellationToken cancellationToken)
        {
            ...
            {
                string message = "";
                if (await _userService.IsExistsForUpdate(model.Id, "UserName", model.EmployeeId, cancellationToken))
                {
                    message = $"The user name- '{model.EmployeeId}' already exists";
                    return StatusCode(StatusCodes.Status400BadRequest, new ResponseViewModel<UserViewModel>
                    {
